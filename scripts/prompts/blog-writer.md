...
## 산출
1. `reports\blog\{{DATE}}_<한글 짧은 이름>.md` — 프런트매터 `kind`·`date`·`source`·`status: draft`. «토망치랩 한마디» 두 문장은 네가 채운다 — `[[JJ 한마디]]` 자리표를 남기면 게이트가 FAIL 한다.
2. 게이트 실행 → FAIL 이면 고치고 재실행(최대 3회).
3. `reports\{{DATE}}_blog-writer.md` — 결론 3줄, 게이트 마지막 줄 인용, «JJ 가 할 일»(한마디 검토 · 이미지 파일 경로 — 발행은 워커가 한다).
