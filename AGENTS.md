...
- **세션 첫 일 — 안 읽힌 🔴 를 확인한다 (2026-09-13 신설).** `SessionStart` 훅이 `session_brief.py --hook`(미처리 intent + 오늘·어제 회차 실패)을 부르고, 읽을 것이 있을 때만 **JSON 맥락 + `logs\hooks\codex-brief.txt` 두 길로** 내보낸다. 🔴 **맥락 주입이 실제로 도착하는지는 확인 불가**(코덱스 SessionStart 응답 꼴이 문서에 없다) — 그러니 **그 파일을 직접 열어 보고 시작한다.** 파일이 없으면 읽을 것이 없다는 뜻이다. 끄는 표식은 intent 파일의 «JJ 판정» 칸 `- [x]` 하나뿐이고, **고치지 않은 채 체크하지 않는다.**
