...
- **동시 기동은 사고다 — 시작 지연을 분산하고 pull 은 재시도한다 (2026-08-19 신설).** 네 작업 모두 «놓친 실행은 가능해지면 즉시»(StartWhenAvailable)라, **부팅 뒤엔 같은 초에 넷이 함께 뜬다.** 그러면 한 작업 트리에서 `git pull` 셋이 경쟁해 하나만 이기고 나머지는 `incorrect old value provided` 로 죽는다.
  ...
  - 🔴 **«절전»이 아니라 «종료»였다 (2026-08-22 정정).** 위 근거를 오래 «절전 → 기상»으로 적어 뒀는데 **전제가 틀렸다.** 이 PC 는 **Fast Startup 이 켜진 채 종료**되고, 그때 남는 `Kernel-Power 42` 가 절전 진입처럼 보였을 뿐이다. 7일 실측 — 기동 12회 **전부 부팅**(하이브리드 9 · 콜드 3)이고 **절전 복귀·최대절전 복귀는 0회**다(`Kernel-Boot` id=27 `BootType`). **조항 자체는 그대로 유효하다** — 부팅 뒤에도 넷이 같은 초에 뜬다. 바뀌는 것은 **원인 표기**뿐이다. 이 정정이 없으면 다음 사람이 «절전을 막으면 되겠다»는 엉뚱한 대책으로 간다.
  - 근거: 2026-08-19. 02:12 저전력 진입 → 11:40 **부팅** → 11:46:15 넷 동시 기동, ops-auditor·job-scout 가 `FAIL git-sync` 로 리포트 없이 끝났다. 스크래치 클론에서 재현하니 재시도 없이 12회 중 7회 같은 오류, 재시도 2회로 12/12 성공.
