...
## 2-3. study-scout 등록 (2026-09-05 추가 · **사람 자리** · JJ «붙이자» · **등록 완료 2026-09-05**)

🔴 **여러 줄 블록을 콘솔에 붙이면 창 너비에서 접혀 인자가 끊긴다 (2026-09-05 실측).** `-ExecutionTimeLimit` 과 `-Principal … -Force` 가 다음 줄로 밀려 «명령이 아니다» 오류가 났고, 작업은 **Logon=Interactive · Limit=PT72H** 로 절반만 등록됐다. 조회 없이 «등록했어» 로 끝났으면 일요일 회차가 절전 중 안 돌았을 것이다. **그래서 등록 블록은 `.ps1` 파일로 저장해 `powershell -File` 로 돌린다** — `logs\reg_study.ps1` 이 그 꼴이고, 등록 뒤 실값을 한 줄로 찍는다. 이 절의 블록도 파일로 옮겨 돌리는 것이 정석이다(2-1·2-2 도 같다).
