...
> **성격**: 조사·설계. 🟢 **구현 승인됨 (2026-08-27 JJ)** — 착수는 **별 세션**으로 분리한다(이 문서는 그대로 두고 표기만 남긴다). 아래 §6 범위대로 진행하고, **가드 자체 검사기(§4)를 같이** 만든다. 이 문서는 «무엇이 실제 실패 모드인가»와 «훅으로 막을 수 있는가·그 훅이 조용히 죽지 않는가»까지 확인한 결과다.
