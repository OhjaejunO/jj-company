...
>
> 🔴 **2026-09-13 — 아래 표는 그 회차의 «기록»이고 정본이 아니다.** 손으로 적은 행 번호(`skill_drift_audit.py 258·391행`)는 그 뒤 코드가 움직이면서 **틀린 값이 됐다.** 지금 도는 것을 알려면 **`py scripts\where.py --checks`** 를 부른다 — 자체 검사 전수와 «자동으로 도는 자리가 있는가»를 실물에서 낸다(§0 «실물이 정본이고 문서는 조회 결과다»). 그 자가 첫 실행에서 **손 깃발로만 도는 검사 6개**를 찾아냈고, 그중 본사 5개는 그 자리에서 pre-commit 에 등록했다.
...
| 검사 | 언제 도나 | 무엇을 지키나 |
|---|---|---|
| `run_audit._selftest()` (신설) | **`main()` 진입부** — 매일 12:30 vault-health 래퍼가 부른다 | 회차 판정 네 축(완주 / 무기록 / FAIL 사유 / **«OK (부분:)» 은 FAIL 이 아님**). 마지막 케이스가 없으면 부분 완주가 매일 거짓 🔴 가 된다 |
| `xreview._selftest()` (신설, tomangchi-skill) | **`main()` 진입부** — 검수를 돌릴 때마다 | 출력 형식 판정. 통과 2 · 걸림 3(추론 머리말 · 건수 불일치 · 지적 뒤 총평)이고, 걸림 케이스는 8/26 ep33·ep34 에 실제로 나온 꼴이다 |
| `permission_probe.py --self-test` (신설) | ❌ **회차마다 돌지 않는다 (호출 비용)** — 프로브를 고칠 때 손으로 돌린다 | 프로브가 헛돌지 않는가. deny 경로를 허용한 느슨한 목록으로 돌려 **FAIL 이 나오는지** 본다 |

- **프로브 본체는 매 회차 돈다** — 에이전트를 쓰는 스케줄 셋(vault-health·tomangchi-scout·job-scout)이 에이전트보다 먼저 부르고, 실패하면 그 회차가 중단된다. 위 표의 `--self-test` 는 «검사기를 검사하는» 한 겹 바깥이다.
...
| 검사 | 언제 도나 | 무엇을 지키나 |
|---|---|---|
| `blogcheck.self_test()` | **pre-commit** (`scripts/blogcheck.py` 가 스테이지되면) | 블로그 규격 20여 축 — 2026-09-11 까지 **부르는 자리가 없었다**(«잠복»). 같은 날 `[AD]` 광고 표기 축이 붙으면서 등재했다 |
| `class101._self_test()` (신설) | **pre-commit** (`scripts/class101.py` 가 스테이지되면) | 공개 카탈로그 조회기 10건 — 🔴 `plus=False`(단품만)와 `plus=None`(전체)가 갈리는가 · GraphQL `errors`·`data` 부재에서 **던지는가**(빈 결과로 조용히 안 넘어간다) · 구독/단품 양쪽 매핑 · `plus_of` 가 못 찾으면 `None`(«구독 아님»이 아니다). 🔴 망은 안 탄다 — 「200 이 오는가」는 실물 실행으로 사람이 본다 |
| `naver_blog._self_test()` (신설) | **pre-commit** (`scripts/naver_blog.py` 가 스테이지되면) | 남의 블로그 HTML 파서 넷 — 중첩 `div` 를 끝까지 세는가 · 본문 칸이 없으면 **«못 찾음»이라 적는가**(조용히 빈 글로 안 넘어간다) · 검색 순서 보존 · 대가성 표기 탐지 양방향. 🔴 망은 안 탄다 — 「200 이 오는가」는 실물 실행으로 사람이 본다 |
